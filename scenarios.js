...
const scenarios = [
...

  {
    ...
    // 16턴은 딱 걸치는 값이었다. 정규에서만 마지막 턴에 간신히 닿고, 신병·노련·정예에서는
    // 17턴이 걸려 기한 판정이 방어 쪽으로 넘어갔다 - 아무것도 안 한 추축군이 이겼다.
    // 여유 2턴을 둔다. 난이도를 바꿔도 판정이 뒤집히지 않는 값이다.
    turnLimit: 18,
...

  {
    ...
    // 신병에서 연합군이 거점 둘을 잡는 데 15턴이 걸렸다. 기한이 14턴이라 한 턴 차이로
    // 기한 판정이 추축군에게 넘어갔다 - 엘 알라메인과 같은 모양이다. 같은 이유로 2턴 올린다.
    turnLimit: 16,
    startDate: [1944, 6, 22],
    timeoutWinner: "east",
    map: { enabled: true },
